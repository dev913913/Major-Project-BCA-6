 ...
 * Uses setState and useEffect with window.matchMedia to reactively respond to resize events.
 *
...
function useMediaQuery(query) {
  ...
  useEffect(() => {
    // Only run on client side - prevents SSR hydration mismatch
    ...
    // Add listener (support both old and new API for broad browser compatibility)
 ...
 * Markdown body, and any code snippets. Memoized and receives pre-processed snippets
 * to prevent unnecessary re-renders and duplicate calculations.
 *
 * @param {Object} props
 * @param {string} props.title - Lesson title.
 * @param {string} props.content - Lesson body in Markdown.
 * @param {string[]} props.snippets - Array of pre-processed, trimmed code snippet strings.
 ...
 *
 * Responsive behavior:
 * - Mobile (<768px): Tab-based navigation showing one panel at a time
 * - Tablet/Desktop (≥768px): Two or three-column layout with all panels visible
 *
...
function LessonsManagerPage() {
  ...
  // Form reference for native HTML5 validation via requestSubmit()
  const formRef = useRef(null);

  // Responsive hook for md breakpoint (768px) - reactive and SSR-safe
   ...
   * HTML5 validation runs before `handleSubmit` is called. This ensures required
   * fields are validated before submission, preventing incomplete lesson records.
   * Used by the mobile sticky action bar's Save/Update button.
   */
  function handleMobileSave() {
    if (formRef.current) {
      // Use requestSubmit() to trigger proper HTML5 validation
      // This ensures title and content are not empty before submission
      formRef.current.requestSubmit();
    }
  }

  // Memoize processed snippets to avoid duplicate calculations on every render
  const processedSnippets = useMemo(
    () => mapTextareaToCodeSnippets(form.code_snippets),
    [form.code_snippets]
  );
  ...
  return (
    <div className="space-y-6 pb-32 md:pb-0">
      ...
      {/* Mobile tab bar - only visible on screens < 768px */}
      ...
      {/* Desktop / Tablet controls to toggle visibility - only visible on screens >= 768px */}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        {showPreview && (activeTab === 'preview' || isDesktop) && (
          <section className="space-y-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <PreviewContent title={form.title} content={form.content} snippets={processedSnippets} />
          </section>
        )}

...
function LessonPage() {
  ...
  useSeo({
    title: safeLesson ? `${safeLesson?.title ?? ''} | Codev by Dev Kumar` : 'Lesson | Codev by Dev Kumar',
    description: lessonDescription(safeLesson?.content ?? ''),
    type: 'article',
    image: safeLesson?.featured_image,
  ...
  const codeSnippets = useMemo(
    () => normalizeCodeSnippets(safeLesson?.code_snippets ?? []),
    [safeLesson?.code_snippets],
  );
  const readingTime = useMemo(() => wordsToMinutes(safeLesson?.content ?? ''), [safeLesson?.content]);

  if (error) return <ErrorState message={error} onRetry={loadLesson} />;
  if (!safeLesson) return <div className="h-64 animate-pulse rounded-2xl bg-slate-100" />;

  return (
    <article className="space-y-8">
      <JsonLd
        id={`lesson-jsonld-${safeLesson?.id ?? 'unknown'}`}
        data={{
          '@context': 'https://schema.org',
          '@type': 'Article',
          headline: safeLesson?.title ?? '',
          author: { '@type': 'Person', name: 'Dev Kumar' },
          datePublished: safeLesson?.created_at,
          dateModified: safeLesson?.updated_at,
          image: safeLesson?.featured_image,
          articleSection: safeLesson?.categories?.name,
        }}
      />
      ...
      <header className="relative overflow-hidden rounded-3xl bg-slate-900 text-white">
        {safeLesson?.featured_image && (
          <img src={safeLesson?.featured_image} alt={safeLesson?.title ?? ''} className="h-72 w-full object-cover opacity-40" loading="lazy" />
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/70 to-slate-900/20" />
        <div className="absolute inset-0 flex flex-col justify-end p-6 sm:p-10">
            <span className="mb-3 inline-flex w-fit rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide">{safeLesson?.categories?.name ?? 'General'}</span>
          <h1 className="max-w-4xl text-3xl font-black leading-tight sm:text-5xl">{safeLesson?.title ?? ''}</h1>
          <div className="mt-4 flex flex-wrap gap-3 text-sm text-slate-200">
            <span>📅 {formatLessonDate(safeLesson?.created_at)}</span>
            <span>👁 {safeLesson?.views_count ?? 0} views</span>
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr,280px]">
        <div className="min-w-0 space-y-6">
          <MarkdownRenderer content={safeLesson?.content ?? ''} />
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-24 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <ul className="mt-3 space-y-2 text-sm text-slate-600">
              ...
              <li>Category: {safeLesson?.categories?.name ?? 'General'}</li>
